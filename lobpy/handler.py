...
def calibrate_mrevdynamics_lobster_rf(
    ...
    if cal_to_average:
        dt, time_stamps, volume_bid, volume_ask = lobreader.load_marketdepth(
            num_observations=timegrid_size,
...
def extract_volume_lobster(
    ...
    args:
        ...
        num_levels_data,
        num_levels_calc,
        timegrid_size       if None, then the process is extracted for all time points in the data, else a uniform grid is created
...
def extract_price_lobster(
    ...
    print('Extracting total volume process.')    
    dt, time_stamps, prices_bid, prices_ask = reader.load_prices(timegrid_size, write_output=True)
...
def _prediction_rvar_running_frame(
    ...
    # model1= lobprice.PriceModel(modelid=lob.create_lobster_filename(ticker_str, date_str, str(time_start_calc), str(time_end_calc), "price-model-rcg", str(num_levels_calc)))
    # cal1 = lobm.OrderVolumeCalibrator(
    #     calibratorid=lob.create_lobster_filename(ticker_str, date_str, str(time_start_calc), str(time_end_calc), "cal-price-model-rcg", str(num_levels_calc)),
    #     model=model2,
    #     estimator_dynamics=None,
    #     estimator_full=estimate_vol_2d_rv_incr            
    # )
    # # Set up model and calibrator using autocorrelation
    # model2 = lobprice.PriceModel(modelid=lob.create_lobster_filename(ticker_str, date_str, str(time_start_calc), str(time_end_calc), "price-model-rvar", str(num_levels_calc)))
    # cal2 = lobm.OrderVolumeCalibrator(
    #     calibratorid=lob.create_lobster_filename(ticker_str, date_str, str(time_start_calc), str(time_end_calc), "cal-price-model-rvar", str(num_levels_calc)),
    #     model=model2        
    # )
...
def vol_estimation(
    ...
    __, __, prices_bid, prices_ask = lobreader.load_prices(timegrid_size,write_output=False)
    ...
    filename=lob.create_lobster_filename(ticker_str, date_str, str(time_start_calc), str(time_end_calc), "cal-vol-pred", str(num_levels_calc))
