...
class PriceModel(OrderVolumeMeanRev):
    ...
    def __init__(
            ...
            num_levels=2,
            p0=0.,
            impact_coefficient=1 / float(100)
    ):
        """
    ...
    def get_vol(self):
        ...
        price_vol = (math.sqrt((math.pow(sigma[0], 2) + math.pow(sigma[1], 2) - 2 * sigma[0] *
                                sigma[1] * self.rho)) / float(2))
        return (price_vol * self.impact_coefficient)
    ...
    def simulate(self, dt, num_tpoints, out=None, ret_depths=False):
        ...
        if not ret_depths and (mu[0] == 0) and (mu[1] == 0):
            return _simulate_price_bachelier(dt, num_tpoints, self.p0, (nu[1] - nu[0]) / float(2),
                                             vol)
