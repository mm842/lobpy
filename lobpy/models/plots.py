...
def plot_avprofile_gamma(filename, time_stamps, gammas_bid, gammas_ask,
                         labels_leg=["LSQ", "LSQF", "ArgMax", "$R_{\infty, 1}$"],
                         title_str="Fit of average profile"):
    """ 
    ...
    if num_methods > 4:
        warnings.warn(
            "More than 4 methods provided for gamma not supported. Columns > 4 will not be plotted.",
            RuntimeWarning)
        num_methods = 4
...
def plot_av_profile(profile_bid, profile_ask, filename, ticker_str, date_str, time_start, time_end):
    ...
    ax.set_title(
        'Average Profile\nTicker: {0}, Date: {1}\n Time: {2} to {3}'.format(ticker_str, date_str,
                                                                            time_start, time_end))
...
def plot_calibration_history_volume(cal_history, filename="calhistoryplot", titlestr="",
                                    plot_corr=True):
    ...
    moments_bid = (np.divide(cal_history.params_bid['nu'],
                             np.power(cal_history.params_bid['sigma'], 2)) * 2. + 1).astype(int)
    moments_ask = (np.divide(cal_history.params_ask['nu'],
                             np.power(cal_history.params_ask['sigma'], 2)) * 2. + 1).astype(int)
    ...
    axs[1].fill_between(time_stamps, np.clip(moments_bid, 0, 4), 4, where=(moments_bid <= 4),
                        facecolor='g', alpha=0.5)  # interpolate=True
    axs[1].fill_between(time_stamps, np.clip(moments_ask, 0, 4), 4, where=(moments_ask <= 4),
    ...
    if plot_corr:
        axs[num_params].plot(time_stamps, cal_history.params_correlation, color='b',
                             label=r'$\rho$')
...
def plot_avprofile_fits(profile_bid, profile_ask, models,
                        labels_leg=["data", "LSQ", "LSQF", "ArgMax", "$R_{\infty, 1}$"],
                        title_str="Fit of average profile", filename="av-profile"):
    """ 
    Plots the resuls from fitting average profiles. Supports up to 4 different methods used for calibration.
    ----------
    args:
    ...
    if num_models > 4:
        warnings.warn(
            "More than 4 models not supported for plooting. Only first 4 will be plotted.",
            RuntimeWarning)
        num_models = 4
