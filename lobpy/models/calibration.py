...
class ParameterHistory():
    ...
    def _check_len(self):
        ...
        for key_bid, list_bid, key_ask, list_ask in zip(self.params_bid.items(),
                                                        self.params_ask.items()):
            if (num_t != len(list_bid)):
                warnings.warn(
                    "Size of time stamps is not consistent with lengths of {}".format(key_ask))
                return False
    ...
    def get_modelpardict_t(self, time_index, modelid="model_from_calhist"):
        ...
        """
        out = {key: (self.params_bid[key][time_index], self.params_ask[key][time_index]) for key in
               self.params_bid.keys()}
...
def read_calhistory_csv(filename):
    ...
    for key_baflag in invals.columns:
        if key_baflag in ("Time", CORRELATION_PAR_STR):
            ...
            warnings.warn(
                "Unkown column found in file: {}. Values will be ignored.".format(key_baflag),
                RuntimeWarning)
            continue
        try:
            ...
            warnings.warn(
                "Unkown column found in file: {}. Expected: key_bid or key_ask. Values will be ignored.".format(
                    key_baflag), RuntimeWarning)
            continue
        if baflag == "bid":
            ...
            warnings.warn(
                "Unkown column found in file: {1}. {2} should be bid or ask. Values will be ignored.".format(
                    key_baflag, baflag), RuntimeWarning)
