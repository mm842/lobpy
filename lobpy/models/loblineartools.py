...
class LOBProfile:
    ...
    def _fit_profile_tv_leastsq_full(self, profile_data):
        ...
        if len_data > .5 * L:
            ...
            print(
                "Warning: Parameter L was too small. Size of data array has been reduced to %i" % len_data)
    ...
    def fit_profile_tv_leastsq(self, profile_data):
        ...
        profile_data_normalized = profile_data / float(
            np.sum(profile_data))  # Normalize data to fit
        ...
        if len_data > .5 * L:
            ...
            print(
                "Warning: Parameter L was too small. Size of data array has been reduced to %i" % len_data)
            profile_data_normalized = profile_data[:len_data:] / float(
                np.sum(profile_data[:len_data:]))

        def res_profile(par):
            """ residual profile to fit to """
            gamma = par[0]
            x_arr = np.linspace(1, len_data, len_data) - .5
            res = profile_data_normalized - np.sin(np.pi / L * x_arr) * np.exp(
                -gamma * x_arr) / self._profilemass(gamma)
            return res
...
class LOBProfileScaled:
    ...
    def _profile_model_scaled(self, xlevels, gamma, z0, alph):
        ...
        return z0 * np.sin(np.pi / self.pmax * np.power(xlevels, alph)) * np.exp(
            -gamma * np.power(xlevels, alph))
    ...
    def _profile_model_scaled_jac(self, xlevels, gamma, z0, alph):
        ...
        ddgamma = - xlevels_rescaled * z0 * np.sin(np.pi / self.pmax * xlevels_rescaled) * np.exp(
            -gamma * xlevels_rescaled)
        ddz0 = np.sin(np.pi / self.pmax * xlevels_rescaled) * np.exp(-gamma * xlevels_rescaled)
        ddalph = -gamma * z0 * np.sin(np.pi / self.pmax * xlevels_rescaled) * np.exp(
            -gamma * xlevels_rescaled)
        ddalph += np.pi / self.pmax * z0 * np.cos(
            np.pi / self.pmax * np.power(xlevels, alph)) * np.exp(-gamma * xlevels_rescaled)
    ...
    def fit_profile_leastsq_scaling(self, profile_data):
        ...
        try:
            ...
            print(e)
            warnings.warn(
                "Optimization failed due to an exception. Note that Jacobi supported from scipy version 0.18 onwards. The current scipy version is: {}".format(
                    scipy.__version__))
            try:
                par = sopt.curve_fit(
                    self._profile_model_scaled,
                ...
                print(e)
                warnings.warn(
                    "Optimization failed due to an exception. Running optimization without boundary constraints on the parameters now. Please check the results carefully. Note that bounds are supported from scipy version 0.17 onwards. The current scipy version is: {}".format(
                        scipy.__version__))
    ...
    def fit_profile_leastsq_scaling_1(self, profile_data):
        ...
        def res_profile(par):
            ...
            alph = par[2]
            x_arr = np.linspace(1, len_data, len_data) - .5
            res = profile_data - - z0 * np.sin(np.pi / L * np.power(x_arr, alph)) * np.exp(
                -gamma * np.power(x_arr, alph))
            return res
