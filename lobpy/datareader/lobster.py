...
def _split_lobster_filename(filename):
    ...
    ticker_str, date_str, time_start_str, time_end_str, file_type_str, num_levels = filename2.split(
        "_")
...
def _create_lobster_filename(ticker_str, date_str, time_start_str, time_end_str, file_type_str,
                             num_levels):
    return "_".join((ticker_str, date_str, time_start_str, time_end_str, file_type_str, num_levels))


def create_lobster_filename(ticker_str, date_str, time_start_str, time_end_str, file_type_str,
                            num_levels):
    return _create_lobster_filename(ticker_str, date_str, time_start_str, time_end_str,
                                    file_type_str, num_levels)
...
def _get_time_stamp_before(time_stamps, time_stamp):
    ...
    if time > time_stamp:
        raise LookupError(
            "Time stamp data start at {} which is after time_stamps: {}".format(time, time_stamp))
...
class LOBSTERReader(OBReader):
    ...
    def __init__(self, ticker_str, date_str, time_start_str, time_end_str, num_levels_str,
                 time_start_calc_str="", time_end_calc_str="", num_levels_calc_str=""):
        super().__init__()
        self.ticker_str = ticker_str
        self.date_str = date_str
        self.lobfilename = _create_lobster_filename(ticker_str, date_str, time_start_str,
                                                    time_end_str, ORDERBOOK_FILE_ID, num_levels_str)
        self.msgfilename = _create_lobster_filename(ticker_str, date_str, time_start_str,
                                                    time_end_str, MESSAGE_FILE_ID, num_levels_str)
    ...
    def average_profile_tt(self, num_levels_calc_str="", write_outputfile=False):
        ...
        with open(self.lobfilename + ".csv", newline='') as csvfile:
            ...
            if not write_outputfile:
                return mean[1::2], mean[
                                   0::2]  # LOBster format: bid data at odd * 2,  LOBster format: ask data at even * 2

            print("Write output file.")
            outfilename = self.create_filestr("-".join(("ttime", AV_ORDERBOOK_FILE_ID)),
                                              str(num_levels_calc))
    ...
    def average_profile(
        ...
        with open(".".join((self.lobfilename, 'csv')), newline='') as orderbookfile, open(
            ...
            for rowLOB, rowMES in zip(lobdata,
                ...
                if flag == 0:
                    if time_start <= nexttime:
                        ...
                        for ctr, currbucket in enumerate(currprofile):
                            tempval1 = (nexttime - currtime) / float(
                                time_end - time_start) * currbucket - comp[ctr]
                            tempval2 = mean[ctr] + tempval1
                            comp[ctr] = (tempval2 - mean[ctr]) - tempval1
                            mean[ctr] = tempval2
                else:
                ...
                for ctr, currbucket in enumerate(currprofile):
                    # print(lobstate)
                    tempval1 = (nexttime - currtime) / (time_end - time_start) * currbucket - comp[
                        ctr]
                    tempval2 = mean[ctr] + tempval1
                    comp[ctr] = (tempval2 - mean[ctr]) - tempval1
                    mean[ctr] = tempval2
    ...
    def _load_ordervolume_full(
        ...
        if ret_np:
            return np.array(time_stamps[index_start:index_end]), np.array(
                volume_bid[index_start:index_end]), np.array(volume_ask[index_start:index_end])
        return time_stamps[index_start:index_end], volume_bid[index_start:index_end], volume_ask[
    ...
    def _load_profile_snapshot_lobster(
        ...
        with open((self.lobfilename + '.csv')) as orderbookfile, open(
            ...
            if time_file > time_stamp:
                raise LookupError(
                    "Time data in the file start at {} which is after time_stamps: {}".format(
                        time_file, time_stamp))
            if time_file == time_stamp:
            ...
            for rowMES in messagedata:
                ...
                if time_file > time_stamp:
                    # file format is [ask level, ask volume, bid level, bid volume, ask level, ....]
                    # conversion of price levels to USD
                    bid_prices = np.fromiter(rowLOB[2:(4 * num_levels_calc):4], np.float) / float(
                        10000)
                    bid_volume = np.fromiter(rowLOB[3:(4 * num_levels_calc):4], np.float)
                    # conversion of price levels to USD
                    ask_prices = np.fromiter(rowLOB[0:(4 * num_levels_calc):4], np.float) / float(
                        10000)
                    ask_volume = np.fromiter(rowLOB[1:(4 * num_levels_calc):4], np.float)
                    break
